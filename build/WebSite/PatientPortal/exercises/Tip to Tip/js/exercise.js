...
Leap.loop({background:false, frameEventName:"animationFrame"},function (frame){
    if(Game.isPlayable()){
        if (!Game.isPaused() && !Game.isCompleted()){
                ...
                for(let handIndex = 0, numberOfHands = frame.hands.length; handIndex < numberOfHands; handIndex++){
                    ...
                    if(hand.type == selectedHand){
                         ...
                         if (Game.distanceCheck()) {
							 ...
							 Game.processTracking(repsToDo, trackRecord.seq, fingersSelected[trackRecord.finger], record,trackRecord.lastElement);
							 record=[];
							 recording = false;
                         }
					}
            ...
            for (let handIndex = 0, numberOfHands = frame.hands.length; handIndex < numberOfHands; handIndex++) {
                ...
                if (hand.type == selectedHand) {
					...
					if (Game.distanceCheck()) {
						...
						Game.processTracking(repsToDo, trackRecord.seq, trackRecord.finger, record,trackRecord.lastElement);
						record=[];
						recording = false;
					}
                }
            ...
            if (recording) {
                Game.processTracking(repsToDo, trackRecord.seq, trackRecord.finger, record,true);
                record = [];
                recording = false;
                Game.tick(1,0);
            }
