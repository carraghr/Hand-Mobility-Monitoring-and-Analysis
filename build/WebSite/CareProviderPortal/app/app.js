let careProviderPortal = angular.module('careProviderPortal',['ngCookies','ngRoute','ngAria','ngAnimate', 'ngSanitize', 'ui.bootstrap','ngResource']);

careProviderPortal.config(function ($routeProvider){
    $routeProvider
        .when('/exercises',{
        ...
        .when('/feedback',{
            ...
            controller : 'feedbackDisplay'
        })
});

careProviderPortal.controller('patientCtrl', function patientCtrl($scope, $http, $location, $cookies, $httpParamSerializerJQLike, $document, $uibModal){
    ...
    $scope.addPatient = function(parentSelector){
        let parentElem = parentSelector ? angular.element($document[0].querySelector(parentSelector)) : undefined;
        let modalInstance = $uibModal.open({
                animation: true,
                ariaLabelledBy: 'modal-title',
                ariaDescribedBy: 'modal-body',
                templateUrl: './app/components/patient/addPatientForm.html',
                controller: 'addPatientFormController',
                controllerAs: '$form',
                size: 'lg',
                appendTo: parentElem,
                resolve: {}
    ...
    $scope.getPatientData = function(){
        if($scope.patientID.length > 0) {
            ...
            $http(request).then(function (response) {
                ...
                $scope.show = Object.keys(response.data).length != 0;
            }, function (response) {});
        }
    };
...
careProviderPortal.controller('addPatientFormController', function ($uibModalInstance, $http, $httpParamSerializerJQLike, $document, $cookies) {
    ...
    $form.ok = function() {
        let formData = $('form[name ="addPatientForm"]').serializeArray();
        let request = {
            method: 'POST',
            url: tempLoc + '/assests/php/addPatient.php',
            headers: {'Content-Type': 'application/x-www-form-urlencoded'},
            data: $httpParamSerializerJQLike({ 'PHPSESSID' : $cookies.get('PHPSESSID'), 'form':formData})
        };
        $http(request).then(function (response){
...
careProviderPortal.controller('exerciseScheme', function exercises($scope, $http, $location, $cookies, $httpParamSerializerJQLike, $document, $uibModal){
    ...
    let removeFromAdditionalList = function (name){
        let exercises = $scope.additionalExericseInfo['exercises'];
        for(let i=0;i<exercises.length;i++){
            if(exercises[i].Name === name ){
    ...
    $scope.selectedExerciseToAdd = function(name, parentSelector){
        ...
        modalInstance.result.then(function (addedExerciseName) {
            ...
            removeFromAdditionalList(addedExerciseName)
        }, function(){});
    };

    $scope.selectedExerciseToChange = function(exercise, parentSelector){
        let request = {
            method: 'POST',
            url: location + '/assests/php/exerciseInformationLookup.php',
            headers: {'Content-Type': 'application/x-www-form-urlencoded'},
            data: $httpParamSerializerJQLike({ 'PHPSESSID' : $cookies.get('PHPSESSID'), 'PatientID' : $cookies.get('PatientID'),'Exercise': exercise})
        };
        ...
        $http(request).then(function (response){
            ...
            let modalInstance = $uibModal.open({
                    ...
                    ariaLabelledBy: 'modal-title',
                    ariaDescribedBy: 'modal-body',
                    templateUrl: './app/components/exercises/adjustExerciseForm.html',
                    controller: 'ExerciseChangeFormController',
                    controllerAs: '$form',
                    size: 'lg',
                    appendTo: parentElem,
                    resolve: {
    ...
    $scope.selectedExerciseToRemove = function(exercise){
        let request = {
            method: 'POST',
            url: location + '/assests/php/removeExercise.php',
            headers: {'Content-Type': 'application/x-www-form-urlencoded'},
            data: $httpParamSerializerJQLike({ 'Exercise':exercise.Name,'PHPSESSID' : $cookies.get('PHPSESSID'), 'PatientID' : $cookies.get('PatientID'), 'exercise':exercise})
        };
        $http(request).then(function (response){
            let exercises = $scope.patientExericseInfo.exercises;
            for(let i=0;i<exercises.length;i++) {
                if (exercises[i].Name === exercise.Name) {
                    exercises.splice(i, 1);
                    $scope.patientExericseInfo.exercises = exercises;
                }
            }
        },function(response){
        });
...
careProviderPortal.controller('ExerciseFormController', function ($uibModalInstance, $http, $httpParamSerializerJQLike, $cookies, exercise) {
...
careProviderPortal.controller('ExerciseChangeFormController', function ($uibModalInstance, $http, $httpParamSerializerJQLike, $cookies, exercise) {
    ...
    $form.ok = function () {
        let formData = $('form[name ="ExerciseAdjustmentForm"]').serializeArray();
        let request = {
            method: 'POST',
            url: tempLoc + '/assests/php/adjustExercise.php',
            headers: {'Content-Type': 'application/x-www-form-urlencoded'},
            data: $httpParamSerializerJQLike({ 'PHPSESSID' : $cookies.get('PHPSESSID'), 'PatientID' : $cookies.get('PatientID'),'form':formData})
        };
...
careProviderPortal.controller('reportGeneration', function reports($scope, $http, $location, $cookies, $httpParamSerializerJQLike, $resource,$document, $uibModal){
    ...
    $scope.exportReport = function(patient,type){
        ...
        if(stage.childNodes.length > 0){
					...
					if(stage.childNodes[childNodeIndex].id.includes("reportElement")){
						for(let i2 = 0; i2 < stage.childNodes[childNodeIndex].childNodes.length; i2++){
							if(stage.childNodes[childNodeIndex].childNodes[i2].id.includes("graph")){
								...
								let imageData = source.highcharts().createCanvas(source.clientWidth, source.clientHeight);
			...
			if(type === 'pdf'){
				...
				exportPDF.autoPrint();
				window.open(exportPDF.output('bloburl'), '_blank');
            }
        }
        $scope.page ="";
    };
...
careProviderPortal.controller('graphFormController', function ($uibModalInstance, $http, $httpParamSerializerJQLike, $document, $cookies, exercise, graphCount, reportElementCount) {
...
careProviderPortal.controller('tableFormController', function ($uibModalInstance, $http, $httpParamSerializerJQLike, $document, $cookies, exercise, tableCount, reportElementCount) {
...
Highcharts.Chart.prototype.createCanvas = function(width, height) {
    let svg = this.getSVG(),
...
careProviderPortal.controller('feedbackDisplay', function reports($scope, $http, $location, $cookies, $httpParamSerializerJQLike){
    ...
    let location = $location.absUrl().substring(0,$location.absUrl().lastIndexOf("CareProviderPortal/")+19);
